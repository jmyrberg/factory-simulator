...
def plot_timeline(data, end_dt, **kwargs):
    ...
    df = (
        ...
        .assign(
            ...
            end_ts=lambda df_:
                df_.groupby(['obj', 'key'])['ds'].shift(-1).fillna(end_dt),
    ...
    fig = px.timeline(
        df, facet_row='name', x_start='ds', x_end='end_ts', y='value',
        title='Simulation - Categorical', color='obj',
        height=n_plots * 100, **kwargs
    )
...
def plot_numerical(data, end_dt=None, **kwargs):
    ...
    fig = px.line(
        df, x='ds', y='value', color='obj', facet_row='key',
...
def plot_factory(factory):
    ...
    plot_timeline(
        categorical, end_dt, width=800)
    plot_numerical(
        numerical, end_dt, width=800)
